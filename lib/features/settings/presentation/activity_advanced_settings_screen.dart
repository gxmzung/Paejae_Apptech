// lib/features/settings/presentation/activity_advanced_settings_screen.dart
...
import 'package:apptech_flutter/core/settings/activity_settings.dart';
...
class _ActivityAdvancedSettingsScreenState
  ...
  Future<void> _load() async {
    final s = await ActivitySettingsRepo.load();
    _strideCtrl.text = s.strideMeters.toStringAsFixed(2);
    _goalCtrl.text = s.goalSteps.toString();
    _kcalCtrl.text = s.kcalPerStep.toStringAsFixed(3);
  ...
  Future<void> _save() async {
    final current = await ActivitySettingsRepo.load();

    final stride = _parseDouble(_strideCtrl.text, current.strideMeters);
    final goal = _parseInt(_goalCtrl.text, current.goalSteps);
    final kcal = _parseDouble(_kcalCtrl.text, current.kcalPerStep);
    ...
    await ActivitySettingsRepo.save(next);
